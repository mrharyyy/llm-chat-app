...
function appendMessage(message, sender = "user") { const messageElement = createMessageElement(message, sender); chatMessages.appendChild(messageElement); scrollToBottom(true); }
...
function scrollToBottom(force = false) { const isAtBottom = chatMessages.scrollTop + chatMessages.clientHeight >= chatMessages.scrollHeight - 50;

if (force || isAtBottom) { chatMessages.scrollTop = chatMessages.scrollHeight; } }

function simulateTypingEffect(text, callback) { const messageElement = createMessageElement("", "assistant"); const content = messageElement.querySelector(".content"); chatMessages.appendChild(messageElement); scrollToBottom(true);
...
// ✅ Final working API call async function handleUserMessage(message) { showTypingIndicator(true);

try { const response = await fetch("https://swift-api-bot.sleek.workers.dev/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ messages: [ { role: "user", content: message } ] }) });

const data = await response.json();
showTypingIndicator(false);

let reply = data.response || "Mujhe turant uttar nahi mila.";
...
} catch (err) { showTypingIndicator(false); simulateTypingEffect("⚠️ Koi error aaya. Kripya later try karein."); console.error("Fetch error:", err); } }
